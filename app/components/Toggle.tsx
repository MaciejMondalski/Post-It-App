...
const disableChildClick = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
  e.stopPropagation();
};
...
export default function Toggle({ deletePost, setToggle }: ToggleProps) {
  return (
    ...
    >
      <div
        onClick={(e) => disableChildClick(e)}
        className="absolute bg-white top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-12 rounded-lg flex flex-col flex-center text-center gap-6"
      >
